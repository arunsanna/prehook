...
*[Scene: A developer staring at a Slack message at 2 AM. Their laptop screen glows ominously. A PagerDuty alert banner reads: "AWS keys found in public repo."]*
...
*[Scene: A lineup of "solutions" standing against a wall. Each holds a sign.]*
...
**Narrator:** The problem? Most security scanning happens *after* the code leaves the developer's machine. By then, the secret is already in history.

---

## PANEL 3: Enter prehook

*[Scene: A single binary drops onto a terminal like a superhero landing. The terminal shows three commands.]*
...
**Narrator:** `prehook` runs *before* your code leaves your machine. Not in CI. Not in a review. Right here, right now, on `git commit` and `git push`.

---

## PANEL 4: The Two Gates

*[Scene: Split panel. Left side shows a commit gate, right side shows a push gate. Each has a bouncer checking IDs.]*
...
*Scans only the staged snapshot from the Git index -- not your messy working tree.*
...
*[Scene: A developer opens .prehook.yaml. The file is short and readable. They change one line.]*
...
*[Scene: Terminal output from `prehook doctor`. Green checkmarks and one red X.]*
...
*[Scene: A developer tries to commit. The terminal lights up red.]*
...
*[Scene: A whiteboard with three principles written on it. A team stands around nodding.]*
...
*[Scene: A terminal with a stopwatch showing 47 seconds.]*

```bash
# Install (pick one)
go install github.com/arunlabs/prehook@latest
# or: brew tap arunlabs/tap && brew install prehook
...
*[Scene: The same developer from Panel 1, now sleeping peacefully. Their laptop shows a green terminal: "pre-commit: all gates passed."]*
